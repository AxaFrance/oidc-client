...
export const tryKeepSessionAsync = async (oidc: Oidc) => {
  ...
  try {
    ...
    if (serviceWorker) {
      ...
      const session = initSession(oidc.configurationName, configuration.storage ?? sessionStorage, configuration.login_state_storage ?? configuration.storage ?? sessionStorage);
