...
const isTokensOidcValid = (
	...
	if (tokens.idTokenPayload) {
		...
		if (idTokenPayload && idTokenPayload.nonce) {
			...
			for (const tabId in nonces) {
				const nonce = nonces[tabId];
				if (nonce === idTokenPayload.nonce) {
					hasValidNonce = true;
					break;
				}
			}
			...
			if (!hasValidNonce) {
				return {
					isValid: false,
					reason: `No nonce from the list ${Object.values(nonces).join(
						', '
					)} matches (idTokenPayload nonce) ${idTokenPayload.nonce}`,
				};
			}
		}
	}
...
function _hideTokens(
	...
	if (id_token) {
		...
		if (_idTokenPayload && _idTokenPayload.nonce) {
			let hasNonceInDatabase = getHasNonceInDatabase(currentDatabaseElement, _idTokenPayload);

			if (hasNonceInDatabase) {
				hideNonce(_idTokenPayload, currentDatabaseElement.configurationName);
			}
		}
...
function getHasNonceInDatabase(currentDatabaseElement: OidcConfig, _idTokenPayload: any) {
	let hasNonceIncluded = false;

	for (const nonce in currentDatabaseElement.nonces) {
		if (nonce === _idTokenPayload.nonce) {
			hasNonceIncluded = true;
			break;
		}
	}
	return hasNonceIncluded;
}

function hideNonce(_idTokenPayload: any, configurationName: string) {
